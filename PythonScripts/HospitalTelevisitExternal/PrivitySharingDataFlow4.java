public class PrivitySharingDataFlow4{

            @InitiatingFlow
            @StartableByRPC
            public static class PrivitySharingDataFlow4Initiator extends FlowLogic<SignedTransaction> {
            
            private Party initiator;
            private List<Party> receivers;
            
            public PrivitySharingDataFlow4Initiator( List<Party> receivers) {
         ...
         builder.addCommand(new HospitalTelevisitExternalContract.Commands.PrivitySharingData4(), Arrays.asList(this.initiator.getOwningKey(), this.receivers.get(0).getOwningKey()));
         builder.addAttachment(attachmentHash);
            ...
            @InitiatedBy(PrivitySharingDataFlow4.PrivitySharingDataFlow4Initiator.class)
            public static class PrivitySharingDataFlow4Responder extends FlowLogic<Void>{
                 
            private FlowSession counterpartySession;
    
            //Constructor
            public PrivitySharingDataFlow4Responder(FlowSession counterpartySession) {
