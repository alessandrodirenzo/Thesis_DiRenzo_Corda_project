public class PrivitySharingDataFlow2{

            @InitiatingFlow
            @StartableByRPC
            public static class PrivitySharingDataFlow2Initiator extends FlowLogic<SignedTransaction> {
            
            private Party initiator;
            private List<Party> receivers;
            
            public PrivitySharingDataFlow2Initiator( List<Party> receivers) {
         ...
         builder.addCommand(new HospitalTelevisitExternalContract.Commands.PrivitySharingData2(), Arrays.asList(this.initiator.getOwningKey(), this.receivers.get(0).getOwningKey()));
         builder.addAttachment(attachmentHash);
            ...
            @InitiatedBy(PrivitySharingDataFlow2.PrivitySharingDataFlow2Initiator.class)
            public static class PrivitySharingDataFlow2Responder extends FlowLogic<Void>{
                 
            private FlowSession counterpartySession;
    
            //Constructor
            public PrivitySharingDataFlow2Responder(FlowSession counterpartySession) {
