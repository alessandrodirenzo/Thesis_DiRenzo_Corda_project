public class DecisionSharingDataFlow1{

            @InitiatingFlow
            @StartableByRPC
            public static class DecisionSharingDataFlow1Initiator extends FlowLogic<SignedTransaction> {
            
            private Party initiator;
            private List<Party> receivers;
            
            public DecisionSharingDataFlow1Initiator( List<Party> receivers) {
         ...
         builder.addCommand(new HospitalTelevisitExternalContract.Commands.DecisionSharingData1(), Arrays.asList(this.initiator.getOwningKey(), this.receivers.get(0).getOwningKey()));
         builder.addAttachment(attachmentHash);
            ...
            @InitiatedBy(DecisionSharingDataFlow1.DecisionSharingDataFlow1Initiator.class)
            public static class DecisionSharingDataFlow1Responder extends FlowLogic<Void>{
                 
            private FlowSession counterpartySession;
    
            //Constructor
            public DecisionSharingDataFlow1Responder(FlowSession counterpartySession) {
